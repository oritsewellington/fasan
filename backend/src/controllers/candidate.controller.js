...
import { buildUrl, deleteFile, resizeAndConvert } from '../middleware/upload.middleware.js';

// GET /api/events/:eventId/candidates
export async function getCandidates(req, res) {
  const candidates = await Candidate.find({ event: req.params.eventId }).sort('candidateNumber');
  res.json(candidates);
}

// GET /api/events/:eventId/candidates/:candidateId
export async function getCandidate(req, res) {
  const candidate = await Candidate.findOne({ _id: req.params.candidateId, event: req.params.eventId });
  if (!candidate) return res.status(404).json({ message: 'Candidate not found.' });
  res.json(candidate);
}

// POST /api/events/:eventId/candidates  (admin OR the event's organizer)
export async function createCandidate(req, res) {
  ...
  // Organizers can only add candidates to their own events
  if (req.user.role === 'organizer') {
    const ownerId = (event.organizer || event.organizerId)?.toString();
    if (ownerId !== req.user._id.toString())
      return res.status(403).json({ message: 'You can only add candidates to your own events.' });
  }

  // Auto-generate unique candidate number within this event
  const lastCandidate = await Candidate.findOne({ event: event._id }).sort('-candidateNumber');
  const num = lastCandidate ? lastCandidate.candidateNumber + 1 : 1;

  // Process photo
  let photo = '', photoFilename = '';
  if (req.file) {
...
// PUT /api/events/:eventId/candidates/:candidateId  (admin OR event's organizer)
...
// DELETE /api/events/:eventId/candidates/:candidateId  (admin OR event's organizer)
