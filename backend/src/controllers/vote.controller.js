...
export async function initializePayment(req, res) {
  ...
  if (!event.isOpen || now < event.startDate || now > event.endDate)
    return res.status(400).json({ message: 'Voting is not currently open for this event.' });

  const candidate = await Candidate.findOne({ _id: candidateId, event: eventId });
  if (!candidate) return res.status(404).json({ message: 'Candidate not found.' });

  const totalAmount  = event.pricePerVote * quantity;
  const commission   = event.platformCommission;
  const platformCut  = Math.round(totalAmount * commission);
  const organizerCut = totalAmount - platformCut;

  const existingRef = await Vote.findOne({ reference });
  if (existingRef) return res.status(409).json({ message: 'Duplicate payment reference.' });

  await Vote.create({
    voterName: name, voterEmail: email,
    event: eventId, candidate: candidateId,
    eventTitle: event.title, candidateName: candidate.name,
    organizerId: event.organizer,
    votes: quantity, amount: totalAmount,
    platformCommission: commission, platformCut, organizerCut,
...
export async function verifyPayment(req, res) {
  ...
  if (!vote) return res.status(404).json({ message: 'Payment reference not found.' });
  if (vote.status === 'verified') return res.json({ message: 'Already verified.', vote });
  if (vote.status === 'failed')   return res.status(400).json({ message: 'Payment already failed.' });
  ...
  try {
    ...
    vote.status = 'failed'; await vote.save();
    return res.status(502).json({ message: 'Could not reach Paystack to verify payment.' });
  }
  ...
  await User.findByIdAndUpdate(vote.organizerId, {
    $inc: { totalEarnings: vote.organizerCut },
  });
